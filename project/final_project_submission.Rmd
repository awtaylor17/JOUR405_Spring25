...
By looking at the summary statistics for trump_pct, we can see that Maryland was fairly split on its support for Trump. Some counties, like Garrett County (which has the maximum trump_pct value) overwhelmingly supported Trump, while others, like Prince George's County (which has the lowest trump_pct value) hardly supported Trump. This is supported by the mean and median -- the measures of central tendency -- which hover at around 50.
...
Based on these results, there is a weak relationship between median age and trump_pct. There is some relationship present because the correlation correlation coefficient is about 41, which is just under halfway correlated, and the p-value for the linear regression model is below 0.05, so the model is significant. Where the relationship falters is in the adjusted R-squared, which is quite low at 0.1325. That means that this model only covers about 13% of the data. Overall, this relationship can be seen in the scatterplot, in which the data slightly follows the line of best fit, but there are many outliers present.
