...
The independent variables that are strongly correlated with each other are population and percent white, and income and percent bachelors or higher. The independent variables that are strongly correlated with the dependent variable are percent unemployed and percent white. There is only one relationship that has a small enough correlation that it does not show up on the correlation matrix. That relationship is the one between median age and percent bachelors or higher. When looking at the scatterplots, the plots for percent unemployed and percent white show a fairly clear linear relationship with turnout, but the rest do not. There are some, like population and median age, that have a bit of a linear relationship, but most of relationship is due to outliers in the data. 
...
Overall, this model seems to do a good job of predicting voter turnout. This is because the residuals are very low, meaning that the data mostly matches the model. Additionally, both the multiple R-squared and adjusted R-squared are above 80, which is high and means the model does a good job of explaining the data. Two of the variables, population and income, are insignificant because there coefficient value is low, which means that they only have a small effect on the model. I would say that this is a useful model because it can, to a large extent, predict voter turnout and it explains which variables contribute the most to predicting voter turnout. 
...
In my analysis, and even when I played around with a few other variables, the full model always had the higher adjusted R-squared value. This surprised me because I thought that taking out variables that had a low impact on the model, like population and income could make the model have a higher adjusted R-squared. But, that was not the case. In fact, the highest of the three models I created (Model three) is the only model to include the population and income variables, which suggests that they have some sort of impact on the model, as that model is a couple points higher than the others. 

```{r}
# Model 2
model2 <- lm(turnout ~ pct_bachelors_higher + 
                   pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ population + income + 
                   pct_unemployed + median_age, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_bachelors_higher +
...
You could use this model in a voter participation story by comparing the model's prediction with the actual turnout data. Instead using the words "multiple regression," I could using the phrase "predicted turnout based on previous elections." I think this story would be interesting because it would, firstly, test the accuracy of the model, and secondly, see if we are putting too much emphasis (or not enough) on a particular variable. It could also help us to see if there are any other variables we could of included, but didn't. 
