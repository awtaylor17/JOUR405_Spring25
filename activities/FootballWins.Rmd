...
Yes, this is a useful model because, overall, it does a decent job of predicting wins. The model's p-value is significant and its multiple R-squared suggests that the model covers more than half of the data.
...
Points scored has the largest impact on wins, because it has the highest number correlation. Yards gained has lowest correlation number, so it could be removed, though it's number is about 44, so it's not so insignificant that it should absolutely be cut. 
...
You could use this model to try to predict how a team will perform in a certain game or season. Instead of saying "multiple regression model," I would say something like "based on the trends in the team's previous seasons..." This is because, in the model, we are trying to use the data we have to find out what might happen in the future. 
