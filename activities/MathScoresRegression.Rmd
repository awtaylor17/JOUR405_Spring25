...
There is somewaht of a positive relationship between FARMS percentage and match test scores. So, as the percentage of students in the FARMS program increases, their math test scores increase. However, there is no obvious patter, when looking at the raw data without the line of best fit. 
...
So far, this model is not worth using. It tells us that there is not a clear linear relationship between FARMS percentage and math scores. That is because Adjusted R Squared value is low, so pretty much all of the data can be accounted for using other variables besides the ones we looked at. Also, the min and max and the residuals are large, which means that there are data that are very far away from are model and do not match it well. 
...
The largest differences between the predicted result and the actual math scores is 34.8, with the top four largest differences also being in the 30's.
...
The conclusion I would draw from this model is that FARMS percentage is not a good value to predict math scores on. This is because, in a large amount of the data, there is a difference (and sometimes a significant difference) between the predicted math scores and the actual math scores. This would influence my story pitch about the board member because it would give me some proof to base their claims off of. In other words, the board member says that FARMS value predicts math scores, but the model says otherwise, so I can use that knowledge and the data we've collected to challenge that claim. 
